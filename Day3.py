...
for _ in range(100):
    ...
    for i, line in enumerate(lines):
        ...
        next = lines[i+1] if i < len(lines)-1 else ''
        matches = [[match.start(), match.end(), int(match.group(0))] for match in re.finditer(r'(\d+)', line)]
        syms = [
            prev[max(match[0]-1, 0):min(match[1]+1, len(prev))]
             + next[max(match[0]-1, 0):min(match[1]+1, len(next))]
             + line[max(match[0]-1, 0)]
             + line[min(match[1],len(line)-1)]
